...
/**
 * Plugin Name:     Comment Pie Filter
...
/**
 * Main Comment_Pie_Filter Class.
 *
 * @since 0.1.0
 */
final class Comment_Pie_Filter {

	/**
	 * @var   Comment_Pie_Filter The one true Comment_Pie_Filter
	 * @since 0.1.0
	 */
	private static $instance;

	/**
	 * Main Comment_Pie_Filter Instance.
	 *
	 * Insures that only one instance of Comment_Pie_Filter exists in memory at any one
	 * time. Also prevents needing to define globals all over the place.
	 *
	 * @since   0.1.0
	 * @static  var array $instance
	 * @uses    Comment_Pie_Filter::setup_constants() Setup the constants needed.
	 * @uses    Comment_Pie_Filter::includes() Include the required files.
	 * @uses    Comment_Pie_Filter::hooks() Activate, deactivate, etc.
	 * @see     Comment_Pie_Filter()
	 * @return  object | Comment_Pie_Filter The one true Comment_Pie_Filter
	 */
	public static function instance() {
		if ( ! isset( self::$instance ) ) {
			// Setup the setup
			self::$instance = new Comment_Pie_Filter;
			// Methods
	...
	private function setup_constants() {
		...
		// Plugin version.
		if ( ! defined( 'COMMENT_PIE_FILTER_VERSION' ) ) {
			define( 'COMMENT_PIE_FILTER_VERSION', '0.1.0' );
		}

		// Plugin Folder Path.
		if ( ! defined( 'COMMENT_PIE_FILTER_PLUGIN_DIR' ) ) {
			define( 'COMMENT_PIE_FILTER_PLUGIN_DIR', plugin_dir_path( __FILE__ ) );
		}

		// Plugin Includes Path.
		if ( ! defined( 'COMMENT_PIE_FILTER_INCLUDES_DIR' ) ) {
			define( 'COMMENT_PIE_FILTER_INCLUDES_DIR', COMMENT_PIE_FILTER_PLUGIN_DIR . 'includes/' );
		}

		// Plugin Folder URL.
		if ( ! defined( 'COMMENT_PIE_FILTER_PLUGIN_URL' ) ) {
			define( 'COMMENT_PIE_FILTER_PLUGIN_URL', plugin_dir_url( __FILE__ ) );
		}

		// Plugin Root File.
		if ( ! defined( 'COMMENT_PIE_FILTER_PLUGIN_FILE' ) ) {
			define( 'COMMENT_PIE_FILTER_PLUGIN_FILE', __FILE__ );
		}

		// Plugin Base Name
		if ( ! defined( 'COMMENT_PIE_FILTER_BASENAME' ) ) {
			define( 'COMMENT_PIE_FILTER_BASENAME', dirname( plugin_basename( __FILE__ ) ) );
		}
	...
	private function includes() {
		...
		// Includes.
		foreach ( glob( COMMENT_PIE_FILTER_INCLUDES_DIR . '*.php' ) as $file ) { include $file; }
	}
...
/**
 * The main function for that returns Comment_Pie_Filter
 *
 * The main function responsible for returning the one true Comment_Pie_Filter
 * Instance to functions everywhere.
 *
 * Use this function like you would a global variable, except without needing
 * to declare the global.
 *
 * Example: <?php $plugin = Comment_Pie_Filter(); ?>
 *
 * @since 0.1.0
 *
 * @return object|Comment_Pie_Filter The one true Comment_Pie_Filter Instance.
 */
function Comment_Pie_Filter() {
	return Comment_Pie_Filter::instance();
}

// Get Comment_Pie_Filter Running.
Comment_Pie_Filter();
